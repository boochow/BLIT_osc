...
LOGUESDKDIR = ../../..
PROJECTDIR = .
TOOLSDIR = $(LOGUESDKDIR)/tools
EXTDIR = $(LOGUESDKDIR)/platform/ext
PLATFORMDIR = $(LOGUESDKDIR)/platform/$(PLATFORM)
TPLDIR = $(PLATFORMDIR)/osc/tpl
LDDIR = $(PLATFORMDIR)/osc/ld

CMSISDIR = $(EXTDIR)/CMSIS/CMSIS

SEDSCRIPT = s/(\"platform\"\s*:\s*\")(.+)\"/\1$(PLATFORM)\"/
PLATFORMSTR = $(shell echo $(PLATFORM) | tr '[:lower:]-' '[:upper:]_')

$(info $$PLATFORM is [${PLATFORMSTR}])
...
CMDVARS = CC CXXC LD CP AS AR OD SZ
$(foreach var, $(CMDVARS), \
	$(if $(shell which $($(var))),, \
		$(eval $(var) = $(GCC_BIN_PATH)/$($(var)))))
...
DADEFS = -DSTM32F446xE -DCORTEX_USE_FPU=TRUE -DARM_MATH_CM4 -D$(PLATFORMSTR)
DDEFS = $(DADEFS) -D__FPU_PRESENT
else
DADEFS = -DSTM32F401xC -DCORTEX_USE_FPU=TRUE -DARM_MATH_CM4 -D$(PLATFORMSTR)
DDEFS = $(DADEFS) -D__FPU_PRESENT
endif
...
ifeq (,$(PLATFORM))
all:
	@echo "make PLATFORM=(prologue | minilogue-xd | nutekt-digital)"
else
...
.PHONY: $(MANIFEST)
$(MANIFEST): $(BUILDDIR) $(PROJECTDIR)/manifest.json
	@cat manifest.json | /bin/sed -r '$(SEDSCRIPT)' > $@
